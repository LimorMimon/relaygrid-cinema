...
> 2. **A Partner Track integration** — Grafana (the declared track) is **live on the public URL itself**: the official `mcp-grafana` MCP server actually runs inside Vercel's Linux serverless functions (a vendored Linux binary, `vendor/mcp-grafana-linux-x64/` — Grafana Cloud's own hosted MCP endpoint turned out to require interactive OAuth with no machine-to-machine option, so it wasn't usable from a headless function) for real tool-calling, and pushes every event straight into a real Grafana Cloud Loki stream. Confirmed live on `relaygrid-cinema.vercel.app`: 81 tools registered, and a real `Approve & Execute` click landed real log lines in Grafana Cloud with the tab's badge reading "Live". ClickHouse is also a real, fully working integration behind the same `PartnerMcpClient` seam (see "How Gemini connects to MCP" below and `lib/partner-mcp.ts`) — confirmed end to end against a real ClickHouse Cloud service — but it's a Python venv rather than a single binary, so it's **local-dev-only for now**, not live on the public URL, and it isn't the declared track anyway. `PARTNER_MCP` takes a comma-separated list (`clickhouse,grafana` locally; `grafana` alone on Vercel). Every call site treats each partner independently (`getPartnerMcpClients()`), so one having a problem (bad credentials, a subprocess that won't start) is silently dropped for that call and logged server-side — confirmed live: a deliberately broken Loki URL and an invalid partner id both failed without affecting the other partner or the app. In the Integrations tab (`components/sponsor-integrations.tsx`), the Grafana tab asks the server (`app/api/partner-info/route.ts`) which partners are actually configured and labels itself "Live" only when Grafana is truly one of them, falling back to "Simulated" otherwise; the ClickHouse tab's badge is currently a fixed "Live" label rather than a live per-request check — a known asymmetry. The Replit hosting-status preview that used to sit alongside these two was removed as scope reduction (see that file's header comment for why).
