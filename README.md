...
> 2. **A Partner Track integration** — done, twice over: both ClickHouse and Grafana are real, working integrations behind the same `PartnerMcpClient` seam (see "How Gemini connects to MCP" below and `lib/partner-mcp.ts`), each confirmed working end to end against a real cloud service. Only one is active at a time via `PARTNER_MCP` — ClickHouse writes every sponsor event into a real ClickHouse Cloud table via the official `mcp-clickhouse` server; Grafana spawns the official `mcp-grafana` server for real tool-calling and pushes every event straight into a real Grafana Cloud Loki stream. In the Integrations tab (`components/sponsor-integrations.tsx`), the Grafana tab asks the server (`app/api/partner-info/route.ts`) which partner is actually configured and labels itself "Live" only when it's truly the active one, falling back to "Simulated" otherwise; the ClickHouse tab's badge is currently a fixed "Live" label reflecting this app's own primary/default configuration (`PARTNER_MCP=clickhouse`) rather than a live per-request check — a known asymmetry, not a claim that both run simultaneously. The Replit hosting-status preview that used to sit alongside these two was removed as scope reduction (see that file's header comment for why).
...
lib/partner-mcp.ts       Partner MCP client: ClickHouse (live) and Grafana (live) — one active at a time; Replit not implemented
