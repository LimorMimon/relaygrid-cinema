...
> 2. **A Partner Track integration** — done, twice over, and both can run at once: ClickHouse and Grafana are real, working integrations behind the same `PartnerMcpClient` seam (see "How Gemini connects to MCP" below and `lib/partner-mcp.ts`), each confirmed working end to end against a real cloud service. `PARTNER_MCP` takes a comma-separated list (`clickhouse,grafana` by default) — ClickHouse writes every sponsor event into a real ClickHouse Cloud table via the official `mcp-clickhouse` server; Grafana (the declared track on the submission form) spawns the official `mcp-grafana` server for real tool-calling and pushes every event straight into a real Grafana Cloud Loki stream. Every call site treats each partner independently (`getPartnerMcpClients()`), so one having a problem (bad credentials, a subprocess that won't start) is silently dropped for that call and logged server-side — confirmed live: a deliberately broken Loki URL and an invalid partner id both failed without affecting the other partner or the app. In the Integrations tab (`components/sponsor-integrations.tsx`), the Grafana tab asks the server (`app/api/partner-info/route.ts`) which partners are actually configured and labels itself "Live" only when Grafana is truly one of them, falling back to "Simulated" otherwise; the ClickHouse tab's badge is currently a fixed "Live" label rather than a live per-request check — a known asymmetry. The Replit hosting-status preview that used to sit alongside these two was removed as scope reduction (see that file's header comment for why).
